...
export class ChatService implements OnDestroy {
  ...
  private initializeAuthStateListenerSafely(): void {
    ...
    try {
      ...
      // Subscribe to auth state changes with immediate cleanup to prevent stacking
      this.authStateListener = this.firebaseService.onAuthStateChange((user) => {
        console.log('Auth state changed:', user ? 'User logged in' : 'User logged out');
        this.currentUser = user;
        this.isConnectedSubject.next(!!user);
        
        // Immediate cleanup to prevent listener stacking
  ...
  async initializeChat(userId: string, eventId: string, authToken: string): Promise<void> {
    ...
    try {
      ...
      if (!this.isFirebaseReady) {
        ...
        await this.waitForFirebase(5000); // 5 second timeout
      }
   ...
   * Wait for Firebase to be ready with timeout
   */
  private waitForFirebase(timeoutMs: number): Promise<void> {
