/**
 * Test Complete HTTP Token Flow
 * 
 * This script tests the complete flow:
 * 1. Start backend server (if not running)
 * 2. Make HTTP request to get token: GET /chat/token?userId=...&eventId=...
 * 3. Show how to use the token with signInWithCustomToken()
 */
...
async function testHttpTokenGeneration() {
  ...
  try {
    ...
    console.log('URL:', `${BACKEND_URL}/chat/token?userId=${TEST_USER_ID}&eventId=${TEST_EVENT_ID}`);
    
    const response = await axios.get(`${BACKEND_URL}/chat/token`, {
      params: {
        userId: TEST_USER_ID,
...
function showFlowSummary(httpTokenData, directTokenData) {
  ...
  console.log('2. Test HTTP endpoint: curl "' + BACKEND_URL + '/chat/token?userId=' + TEST_USER_ID + '&eventId=' + TEST_EVENT_ID + '"');
...
// Main execution
async function main() {
  console.log('🚀 Complete HTTP Token Flow Test\n');
