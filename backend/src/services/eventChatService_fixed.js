...
class EventChatService {
   ...
   * This method now includes duplicate prevention
   */
  async createEventChatRoom(eventData) {
    try {
      // First, check if a room already exists for this event
      const existingEvent = await Event.findById(eventData._id);
      if (existingEvent && existingEvent.chatRoomCreated && existingEvent.chatRoomId) {
        console.log(`Chat room already exists for event ${eventData._id}: ${existingEvent.chatRoomId}`);
        return {
          success: true,
          data: { roomId: existingEvent.chatRoomId },
          message: 'Chat room already exists for this event'
        };
      }
  ...
  async getEventChatInfo(eventId) {
    try {
      ...
      // Ensure chat room exists - if not, create it safely
      if (!event.chatRoomCreated || !event.chatRoomId) {
        try {
          await this.createEventChatRoom(event);
          // Re-fetch the event to get updated chat room info
          ...
          if (updatedEvent) {
            return {
              ...
              data: {
                ...
                chatRoomId: updatedEvent.chatRoomId,
                chatRoomCreated: updatedEvent.chatRoomCreated,
                participantCount: updatedEvent.attendees.length + 1, // +1 for creator
                isEventActive: updatedEvent.status === 'published' || updatedEvent.status === 'completed',
                eventStatus: updatedEvent.status,
                expiresAt: updatedEvent.expireAt,
          ...
          console.warn('Failed to create missing chat room:', chatError);
          // Continue with the original event data if room creation fails
        }
      }
